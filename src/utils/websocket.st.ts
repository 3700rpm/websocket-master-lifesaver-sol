...
class WebSocketServiceSts {
  ...
  setupSocketListeners(socket: WebSocket, type: any) {
    socket.onopen = () => {
  ...
  on(room: any, listener: any) {
    this.emitter.on(room, listener);
  }
 
  off(room: any, listener: any) {
...
export default WebSocketServiceSts;